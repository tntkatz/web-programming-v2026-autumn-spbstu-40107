...
В модели описан класс User с полями для ID, имени и массива друзей, а также методами для добавления и удаления дружеских связей. Там же реализованы пять независимых функций для обработки коллекции: группировка пользователей по числу друзей с использованием структуры Map, получение общего списка уникальных друзей, а также различные фильтрации (по общим друзьям, по заданному количеству и поиск тех, у кого друзей нет).

Интерфейс динамически генерирует карточки на основе текущего массива пользователей. Все действия — создание или удаление профилей, редактирование списка друзей — происходят без перезагрузки страницы. Для имитации работы с реальным сервером эти операции обернуты в Promise с искусственной задержкой через setTimeout. Каждое изменение автоматически синхронизируется с localStorage, благодаря чему данные полностью восстанавливаются при повторном открытии вкладки.

## Запуск

Запустите локальный веб-сервер в папке с проектом и откройте страницу в браузере.
